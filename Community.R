...
setwd("C:/Users/Luigi/Desktop/LUIGI/DestinE/TASKS/COMMUNITY DATABASE/R Script from Claudia/RUN R/20240408/Files")
...
# Read data from the Community Insight webinar
db06 <- read.csv("20240326_Community_Insights.csv") %>%
  select(`Date`, `Country`,
         `your.name`, `your.surname`, `Organisation`, `your.email`) %>%
  rename(First = `your.name`, Last = `your.surname`,
         Org = `Organisation`, Email = `your.email`) %>%
...
db07 <- read.csv("20240404_Community_Form.csv") %>%
  select(`Date`, `your.country`,
         `first.name`, `Lastname`, `Organisation`, `your.email`) %>%
  rename(Country = `your.country`,
         First = `first.name`, Last = `Lastname`,
...
write.csv(cumulative_data, file = "csv_output/cumulative_registrations.csv", row.names = FALSE)
df_country <- df_country %>% arrange(Country)
write.csv(df_country, file = "csv_output/destine_country_counts.csv", row.names = FALSE)
